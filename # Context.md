...
# MODULE: Social Media Scheduler (Individual & Agency)

## 1. Feature Objective
A direct-integration scheduler allowing users to post content from the AI Skills Studio (Photoshoot/Media Studio) directly to Instagram, Facebook, and TikTok without third-party aggregators.

## 2. Authentication Strategy (OAuth 2.0)
- **Flow:** Individual users connect their accounts via a "Connect Social" button in their Settings.
- **Tokens:** The system exchanges short-lived codes for **Long-Lived User Access Tokens** (60-90 days).
- **Storage:** Tokens are stored in the `social_accounts` table, tied to a specific `workspace_id` to allow for future Agency/Multi-Client scaling.

## 3. Direct API Integration Logic
- **Meta (Instagram/Facebook):** Uses a 2-step publishing process:
  1. Create a media container (POST `/media`) using the asset URL from Supabase Storage.
  2. Publish the container (POST `/media_publish`).
- **TikTok:** Uses a direct POST to the `/video/upload/` endpoint. Note: TikTok requires a separate "Inquiry" status check to ensure the video processed correctly.

## 4. Database Schema: Scheduler Queue
- **Table: `scheduled_posts`**
  - `asset_id`: Link to the photoshoot/video file.
  - `caption`: Text generated by Business Genie.
  - `scheduled_for`: Timestamp.
  - `status`: ['scheduled', 'publishing', 'posted', 'failed'].
- **Automation:** A Supabase Edge Function runs on a 1-minute cron job, checking for posts ready to go live.

## 5. Scaling to Agency Plan
- **Multi-Tenant Logic:** When Agency Mode is active, the `workspace_id` becomes the client identifier.
- **Client Workspaces:** Each client gets a unique workspace. The Agency Admin can toggle between workspaces to manage distinct sets of social tokens without overlap.


# MODULE: Photoshoot (Powered by Nano Banana Pro)

## 1. Feature Objective
This module provides high-fidelity, studio-quality image generation and editing. It utilizes Nano Banana Pro's native reasoning to transform raw uploads into professional assets while maintaining 100% likeness or product integrity.

## 2. Global Nano Banana Pro Logic
Every photoshoot request follows the "Maintain & Modify" architecture:
- **Likeness Anchor:** Nano Banana Pro uses pixel-level semantic masking to "lock" the subject (Face or Product) while rebuilding the surrounding context.
- **Lighting Synthesis:** The model automatically analyzes the light source of the original photo and synthesizes the new environment to match, preventing a "cutout" appearance.
- **High-Fidelity Output:** All final renders utilize the "Pro" reasoning tier to ensure text clarity on labels and realistic skin textures.

---

## 3. MODE A: Product Photography
**Use Case:** Transforming smartphone product shots into commercial-grade marketing assets.

### A. Core Workflow
- **Object Isolation:** Remove original backgrounds while preserving fine edges and transparency.
- **Grounding:** Synthesize realistic contact shadows and surface reflections so the product feels "weighted" in the new scene.

### B. Product Scene Templates
- **Luxe Minimal:** "Professional studio photography of [PRODUCT] on a smooth, white marble pedestal. Background features soft, blurred eucalyptus leaves and warm morning sunlight."
- **Artisanal Kitchen:** "Close-up food photography of [PRODUCT] on a rustic wooden cutting board. Shallow focus with raw ingredients in the background. Warm, overhead kitchen lighting."
- **Modern Tech:** "High-end tech photography of [PRODUCT] on a matte black desk surface. Blurred mechanical keyboard and a soft blue ambient LED strip glow in the background."

---

## 4. MODE B: Professional Headshots
**Use Case:** Converting casual selfies into high-end executive or creative portraits.

### A. Core Workflow
- **Likeness Preservation:** Utilizes Nano Banana's native character-tracking to ensure 1:1 facial feature consistency.
- **Attire Swap:** Automatically replaces the user's clothing with professional attire (suits, blazers, or creative wear) based on the template.

### B. Headshot Scene Templates
- **Executive Boardroom:** "Keep this person's likeness identical. Change the background to a blurred high-end modern office. Change outfit to a professional navy blue tailored suit."
- **Creative Studio:** "Maintain identity consistency. Place the subject in a professional photography studio with a neutral grey textured backdrop and softbox lighting."
- **Outdoor Professional:** "Keep identity exactly the same. Change background to a blurred urban park during golden hour. Apply soft sunlight backlighting."

---

## 5. Technical Guardrails (Universal)
- **Negative Prompting:** Hard-coded prevention of "noisy textures, distorted labels, unreadable text, blurry edges, floating objects, warped reflections, and pixelated artifacts."
- **Smart-Crop:** Automatic 15% padding around the subject with a 2px feather on the mask edges to ensure natural environmental blending.
- **Refinement Pass:** Uses the `Nano Banana Pro Inpaint` function for a secondary detail pass to sharpen eyes (Headshots) or labels (Products).
