...
export default function ContentStepper({ items, lessonId, onAllCompleted }: Props) {
    ...
    async function markCompleted(index: number) {
        ...
        if (newCompleted.size === orderedItems.length) {
            onAllCompleted();
        }

        // Auto-advance if text or action (optional, but good for flow)
        // Actually, text should be manual "Next".
    }
    ...
    function handleActionStart() {
        markCompleted(currentIndex);
        // Redirect to studio with template
        // Stored in 'explanation' field as per LessonBuilder hacking
        // or 'correct_answer_index' if we could store it there, but we used explanation for ID string.
        const templateId = currentItem.content.explanation;
        if (templateId) {
            router.push(`/dashboard/studio?template=${templateId}&intent=remix`);
        } else {
                    ...
                    <div className="absolute inset-0 bg-zinc-900 p-8 flex flex-col items-center justify-center text-center animate-in fade-in">
                        <div className="w-full max-w-lg">
                            ...
                            <button
                                ...
                                className="w-full rounded-lg bg-[#B7FF00] py-4 font-bold text-black hover:bg-[#a3e600] transition transform hover:scale-105 shadow-xl shadow-[#B7FF00]/10"
                            >
