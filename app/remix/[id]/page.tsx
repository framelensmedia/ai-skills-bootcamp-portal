...
export default function RemixPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            // Fetch Profile Separately (to avoid FK issues)
            // Fix: Only select known columns (full_name, profile_image). 
            // 'avatar_url' and 'created_at' might not exist or cause errors if not present.
            ...
            const fullRemixData: RemixDetail = {
                ...
                profiles: profileData ? {
                    ...
                    avatar_url: profileData.profile_image, // Map profile_image to avatar_url prop
                    created_at: "" // No longer fetching profile creation date
                } : {
            ...
            // Load other remixes by this user (Increased limit for Show More)
            ...
            // Strategy: Fetch latest 50, shuffle, take 8
            const { data: randomData } = await supabase
                .from("prompt_generations")
                .select("id, image_url, created_at")
                .eq("is_public", true)
                .neq("user_id", remixData.user_id) // Exclude current creator
                .order("created_at", { ascending: false })
                .limit(50);
            ...
            if (randomData) {
                ...
                setCommunityRemixes(shuffled.slice(0, 8));
            }
                ...
                {communityRemixes.length > 0 && (
                    <div>
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button
                                onClick={() => router.push("/feed")} // Adjust if needed
